...
exports.updateJobStatus = void 0;
...
exports.updateJobStatus = functions.https.onCall(async (data, context) => {
    ...
    const currentStatus = currentJob.status || 'DRAFT';
    const allowedNextStatuses = VALID_TRANSITIONS[currentStatus] || [];
    if (!allowedNextStatuses.includes(targetStatus)) {
        throw new functions.https.HttpsError('failed-precondition', `Cannot transition job from ${currentStatus} to ${targetStatus}`);
    }
    const updates = {
        status: targetStatus,
...
//# sourceMappingURL=jobStateMachine.js.map